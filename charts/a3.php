...
$sql = "SELECT  w.`name` as vendor ,r.violation_amount  FROM crawl_results  r INNER JOIN website w ON r.website_id=w.id INNER JOIN catalog_product_flat_1 p ON p.entity_id=r.product_id  AND p.sku='".$sku."'  WHERE r.crawl_id=".$last_crawl['id']." AND r.violation_amount>1  ORDER BY r.violation_amount DESC LIMIT ".$limit;
$result = mysql_query ( $sql );
